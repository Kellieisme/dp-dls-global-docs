...
The Boeing DLS Global Documentation Hub has a dependency on the Boeing DLS Global Angular Library (`@dasdigitalplatform/dls-global-angular`).

The package `@dasdigitalplatform/dls-global-angular` will be downloaded from the GitLab package registry, as long as the `.npmrc` file is present, as described above.

The package `@dasdigitalplatform/dls-global-assets` will be also downloaded because it is a dependency of `@dasdigitalplatform/dls-global-angular`.

During ongoing development, it may be useful to have local changes made to the `@dasdigitalplatform/dls-global-assets` and `@dasdigitalplatform/dls-global-angular` packages reflect immediately in the `dls-global-docs` project. To enable this, create a NPM symlink.
...
4. In the `dls-global-docs` directory, also run `npm link @dasdigitalplatform/dls-global-angular` to link the local version of `@dasdigitalplatform/dls-global-angular` to the project.
